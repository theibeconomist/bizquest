...
export async function createQuizGame({ subunitId, questions, classId }) {
  const userId = await currentUserId();
  if (!userId) throw new Error("Not signed in.");
  for (let i = 0; i < 8; i++) {
    const code = Math.random().toString(36).slice(2, 8).toUpperCase();
    const { data, error } = await supabase
      .from("quiz_games")
      .insert({ teacher_id: userId, subunit_id: subunitId, join_code: code, questions, class_id: classId || null })
      .select()
      .maybeSingle();
    if (!error) return data;
...
export async function submitQuizAnswer({ gameId, teamId, questionIndex, selectedOption, isCorrect }) {
  const { error } = await supabase.rpc("submit_quiz_answer", {
    p_game_id: gameId, p_team_id: teamId, p_question_index: questionIndex,
    p_selected_option: selectedOption, p_is_correct: isCorrect,
  });
