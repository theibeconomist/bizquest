...
export async function loadRoleInfo() {
  const userId = await currentUserId();
  if (!userId) return { role: "student", classId: null };
  const { data } = await supabase.from("profiles").select("role, class_id").eq("id", userId).maybeSingle();
  return { role: (data && data.role) || "student", classId: (data && data.class_id) || null };
}
...
export async function createClass(name) {
  ...
  for (let i = 0; i < 8; i++) {
    ...
    const { data, error } = await supabase
      .from("classes")
      .insert({ teacher_id: userId, name, join_code: code })
      .select()
      .maybeSingle();
...
export async function loadClassRoster(classId) {
  const { data } = await supabase
    .from("profiles")
    .select("id, email, xp, completed_subunits, class_id")
    .eq("class_id", classId);
  return data || [];
}
